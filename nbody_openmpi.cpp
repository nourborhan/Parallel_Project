...
void calculateForces(double forces[][NDIM], double pos[][NDIM], int n, int id, int p)
{
	...
	// MPI_COMM_WORLD.Allreduce(&forces, &forces, n*NDIM, MPI_DOUBLE, MPI_SUM);

	// sum up forces to id 0
	if (id == 0)
	{
		MPI_Reduce(MPI_IN_PLACE, forces, n*NDIM, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	}
	else
	{
		MPI_Reduce(forces, forces, n*NDIM, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	}
}
